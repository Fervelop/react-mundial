...
export default function Dashboard() {
  ...
  // 1. Estado para controlar el menú móvil sin usar Web Components puros
  const [isOpen, setIsOpen] = useState(false);

  const handleLogout = () => {
    localStorage.removeItem("token");
    navigate("/login"); // Corrige la redirección al login al cerrar sesión
  };
  ...
  return (
    <div className="bg-gray-950 min-h-screen text-white">
      ...
      <header className="absolute inset-x-0 top-0 z-50 bg-gray-900/50 backdrop-blur-md border-b border-gray-800">
        ...
        {/* 2. Menú Desplegable Móvil en React Nativo (Limpio de errores de compilación) */}
        {isOpen && (
