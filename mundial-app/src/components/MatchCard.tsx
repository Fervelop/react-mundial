...
export default function MatchCard({ match }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-5 flex flex-col gap-3 text-gray-900 border border-green-100">
      <div className="flex items-center justify-between gap-3">
        <span className="font-semibold truncate">{match.local}</span>
        <span className="text-xl font-extrabold text-green-600 whitespace-nowrap">
          {match.resultado}
        </span>
      ...
      <div className="flex justify-center">
        <span
          className={`px-3 py-1 text-xs font-semibold uppercase tracking-wide rounded-full ${
            isFinished ? "bg-green-50 text-green-700" : "bg-red-100 text-red-600"
          }`}
        >
          {match.estado}
        </span>
